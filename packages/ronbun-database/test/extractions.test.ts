...
describe("extractions", () => {
  describe("insertExtraction + getExtractionsByPaperId", () => {
    it("inserts and retrieves extractions", async () => {
      await insertExtraction(env.DB, "ext-a", "ep-1", "method", "Transformer", "A neural architecture", "es-1");
      await insertExtraction(env.DB, "ext-b", "ep-1", "dataset", "ImageNet", "Large image dataset", null);
  ...
  describe("searchExtractionsFts", () => {
    it("finds extractions by name via FTS", async () => {
      ...
      await insertExtraction(env.DB, "ext-fts1", "ep-fts", "method", "Transformer", "A neural architecture", null);
    ...
    it("filters by type", async () => {
      ...
      await insertExtraction(env.DB, "ext-fts2", "ep-fts2", "dataset", "ImageNet", "Large dataset", null);
